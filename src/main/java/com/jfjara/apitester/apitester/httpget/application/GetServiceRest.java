...
@Service
public class GetServiceRest implements GetService {

	@Autowired
	private CommunicatorRest communicator;

    @Override
    public Optional<String> send(Map<String, String[]> parameterMap)
            throws NotUrlInParametersException, NotTotalRquestInParametersException,
            CommunicationException, ParametersQueryError, UnsupportedEncodingException {
        ...
        for (int i = 0; i < parametersGroup.getTotalRequestToDo(); i++) {
            optionalsResult.add(communicator.get(parametersGroup.getUrl(), parametersGroup.getParameters()));
        }
        return transformListToOptional(optionalsResult);
    }

    private Optional<String> transformListToOptional(List<Optional<String>> optionalsResult) {
